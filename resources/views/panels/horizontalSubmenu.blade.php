<ul class="dropdown-content dropdown-horizontal-list" id="@if(isset($activate)){{$activate}} @endif">
  @foreach ($menu as $submenu)
    ...
    @endphp
  <li
    class="{{(isset($submenu->submenu)) ? 'dropdown-submenu ' : ''}}{{(request()->is($submenu->slug)) ? 'active ':''}}{{$custom_classes}}"
    data-menu="{{(isset($submenu->submenu)) ? 'dropdown-submenu' : ''}}">
    <a href="{{$submenu->slug}}" class="{{(isset($submenu->submenu)) ? 'dropdownSub-menu' : ''}}"
